...
function M.setup(opts)
    ...
    local has_config = vim.loop.fs_stat(cwd .. "/.astro-settings/settings.toml") ~= nil or
                     vim.loop.fs_stat(cwd .. "/.vscode/sftp.json") ~= nil or
                     vim.loop.fs_stat(cwd .. "/astra.json") ~= nil
...
function M:init_config()
    local cmd = string.format("cd %s && cargo run init", 
...
function M:sync_files(mode)
    local cmd = string.format("cd %s && cargo run sync --mode %s", 
        vim.fn.getcwd() .. "/astra-core", mode)
...
function M:check_status()
    local cmd = string.format("cd %s && cargo run status", 
        vim.fn.getcwd() .. "/astra-core")
...
function M:upload_file(local_path, remote_path)
    local cmd = string.format("cd %s && cargo run upload --local %s --remote %s", 
        vim.fn.getcwd() .. "/astra-core", local_path, remote_path)
...
function M:download_file(remote_path, local_path)
    local cmd = string.format("cd %s && cargo run download --remote %s --local %s", 
        vim.fn.getcwd() .. "/astra-core", remote_path, local_path)
