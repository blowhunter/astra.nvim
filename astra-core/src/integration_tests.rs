...
fn test_incremental_sync_logic() {
    ...
    let local_files: Vec<_> = fs::read_dir(local_dir.path()).unwrap()
        .filter_map(Result::ok)
        .map(|e| e.path())
        .collect();
    
    let remote_files: Vec<_> = fs::read_dir(remote_dir.path()).unwrap()
        .filter_map(Result::ok)
        .map(|e| e.path())
        .collect();
    ...
    let common_local = local_files.iter().find(|p| p.file_name().unwrap() == "common.txt").unwrap();
    let common_remote = remote_files.iter().find(|p| p.file_name().unwrap() == "common.txt").unwrap();
