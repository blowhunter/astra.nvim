...
## 版本 0.4.2 - 完整测试框架与代码优化

### 🧪 测试框架建立

#### 测试套件完整实现
本次更新建立了完整的测试框架，确保代码质量和健壮性：
...
**测试用例覆盖**：
- ✅ Binary 模块：验证、二进制管理、构建功能
- ✅ Config 模块：配置加载、验证、发现功能
- ✅ Sync 模块：上传、下载、同步功能
- ✅ UI 模块：通知、帮助、状态显示
- ✅ Init 模块：状态管理、初始化流程
- ✅ Core 集成：模块间依赖和交互
...
```lua
-- 测试运行器功能
Test.describe("模块名", function()      -- 测试套件
  Test.it("测试描述", function()        -- 单个测试
    Test.assert(condition, "消息")      -- 断言
    Test.skip("跳过原因")                -- 跳过测试
  end)
end)
```
...
**断言函数**：
- `Test.assert(condition, message)` - 基础断言
- `Test.assert_equal(actual, expected)` - 相等断言
- `Test.assert_nil(value)` - nil 断言
- `Test.assert_table(t)` - 表类型断言
- `Test.assert_function(f)` - 函数类型断言

### 🔧 代码优化

#### 安全 Vim API 访问
- **问题**：模块在 headless 测试模式下无法访问 `vim` 全局变量
- **解决**：添加 `safe_vim` 包装器，提供测试模式兼容的 API
- **改进**：所有核心模块现在支持测试环境
...
if vim and vim.fn then
  ...
  safe_vim.fn = {
    ...
    executable = function(path) return os.execute("test -x " .. path) == 0 and 1 or 0 end,
    -- ... 更多模拟函数
  }
end
```
...
#### 架构清理
- **删除** `init_new.lua` - 移除未使用的架构文件
- **统一** `astra.lua` - 重写为主入口文件，直接使用 core 模块
- **精简** `sync.lua` - 删除未注册的函数（upload_selected, upload_multi 等）
...
#### 全新三层架构设计
本次重构引入了全新的三层架构系统，大幅提升代码组织性和用户体验：

**第一层：核心功能抽象层 (Core Functionality)**
- 📦 **模块化设计**：`lua/astra/core/` 包含5个核心模块
  - `init.lua` - 核心初始化和状态管理
  - `binary.lua` - 二进制文件管理（验证、构建、安装）
  - `config.lua` - 配置管理抽象（发现、加载、合并）
  - `sync.lua` - 同步功能抽象（上传、下载、同步）
  - `ui.lua` - UI 和通知抽象（浮动窗口、帮助）

**第二层：公共配置层 (Public Configuration)**
- ⚙️ **智能默认配置**：为常用项目提供合理默认设置
- 🎛️ **功能开关控制**：统一管理同步、通知、过滤等选项
- 🧠 **动态键映射**：根据核心状态自动注册相应命令

**第三层：项目配置层 (Project Configuration)**
- 🎯 **项目级定制**：支持项目特定的配置覆盖
- 📁 **多格式支持**：`.astra.toml`、`.vscode/sftp.json`、`astra.json`
- 🔐 **安全设计**：敏感信息（密码、私钥）项目级覆盖

### 🔧 核心功能改进

#### 严格的状态验证机制
- **三状态系统**：`none`（未初始化）、`basic`（基础功能）、`full`（完整功能）
- **双重验证**：只有同时满足二进制文件和项目配置才开启完整功能
- **渐进式加载**：根据状态级别动态注册命令和键映射

#### 智能二进制文件管理
- **多路径支持**：自动检测本地构建、插件debug/release/static路径
- **版本识别**：显示二进制版本和构建类型信息
- **构建优化**：改进的构建流程和错误处理

#### 配置发现和合并系统
- **优先级机制**：项目配置 > 公共配置 > 默认配置
- **格式兼容**：保持对现有TOML/JSON格式的完全兼容
- **配置验证**：实时验证必要字段和配置完整性

### 🚀 用户体验提升

#### 开箱即用设计
- **零配置启动**：插件在任何目录都能正常启动，不会导致Neovim退出
- **智能提示**：根据当前状态提供相应的操作建议
- **交互式向导**：`:AstraQuickSetup` 提供图形化配置界面

#### 改进的通知系统
- **浮动窗口通知**：现代化的UI设计，支持淡入淡出效果
- **智能去重**：避免重复通知，提升用户体验
- **通知历史**：保留最近的通知记录
...
#### 代码重构
- **模块解耦**：核心功能完全模块化，便于测试和维护
- **状态管理**：集中的状态管理系统，避免全局变量滥用
- **错误处理**：统一的错误处理和用户反馈机制

#### 性能优化
- **缓存机制**：配置发现和二进制验证结果缓存30秒
...
#### 新增命令
- `:AstraQuickSetup` - 交互式快速配置向导
- `:AstraInstall` - 下载预编译二进制文件
- `:Astra` - 统一的主命令接口（check/reload/help）

#### 改进的命令
- `:AstraBuild` - 改进的构建流程和错误提示
- `:AstraInit` - 更好的配置文件生成和管理
- `:AstraHelp` - 现代化的帮助界面，根据功能级别显示相应内容
...
- **模块化测试**：每个核心模块都可以独立测试
- **调试支持**：详细的调试信息和状态检查
- **文档完善**：每个模块都有详细的文档和使用示例

#### 配置示例
- **新配置示例**：`lua/plugins/example_new.lua` 展示新架构配置方法
- **迁移指南**：从旧配置迁移到新架构的详细指南
...
#### 重复通知问题修复
- **核心问题**：用户在上传文件时同时看到成功和失败的重复通知
- **根本原因**：异步任务回调函数（on_stderr 和 on_exit）都被触发，导致多重通知
- **解决方案**：实施基于时间戳的防抖机制，确保2秒内同一文件只有一个通知

#### 用户体验优化
- **通知去重**：上传操作现在只会显示一个最终状态通知
- **界面简化**：移除了 upload_current_file 中的重复通知逻辑
- **状态一致性**：确保用户看到的通知与实际操作结果一致
...
#### 异步回调优化
- **简化逻辑**：重构 upload_file 函数的异步回调处理
- **状态跟踪**：使用模块级变量跟踪最近的通知状态
- **防抖窗口**：设置2秒防抖窗口防止重复通知

### 📦 文件变更
- **lua/astra.lua**: 重构 upload_file 函数通知逻辑，实施防重复机制

### 🧪 测试覆盖
- **通知测试**：验证防重复通知机制在各种情况下正常工作
- **状态验证**：确认只有一个最终状态通知显示给用户
- **边界测试**：测试快速连续上传操作的通知行为
